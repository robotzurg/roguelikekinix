...
if (struct = global.Enemy[eID.ram]) {
	...
	if i_timer = 0{
		...
		hspd_mod = 0;
		if(movetimer <= 0){
			i_timer = max(20, (round((point_distance(x,y,obj_player.x, obj_player.y)/spd)*1.2))); //distance between self and player
			hspd_mod = lengthdir_x(spd,point_direction(x, y, obj_player.x, obj_player.y));
			vspd_mod = lengthdir_y(spd,point_direction(x, y, obj_player.x, obj_player.y));
		}
	}
	hspd += hspd_mod;
	vspd += vspd_mod;
