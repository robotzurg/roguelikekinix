...
/// @requires	rot_prefetch, vec_rot_x, rec_rot_y
/// @author		Lucas Chasteen <lucas.chasteen@xgasoft.com>
/// @copyright	XGASOFT 2020, All Rights Reserved
function device_mouse_check_region_gui() {

	/*
	Checks whether the mouse is currently within a certain region relative to GUI
	...
	argument0 = mouse device or touch point to check, where 0 is first (integer)
	argument1 = horizontal GUI coordinate of the region to check (real)
	argument2 = vertical GUI coordinate of the region to check (real)
	argument3 = rotation of the region to check, in degrees (real)
	argument4 = width (or radius) in pixels of the region to check (real)
	argument5 = height in pixels of the region to check (real) (optional, use no argument for circular)
	argument6 = horizontal alignment of the region to check (constant) (optional, use no argument for fa_center)
	argument7 = vertical alignment of the region to check (constant) (optional, use no argument for fa_middle)
	...
	*/
	...
	if (reg_shape == reg_shape_rectangle) {
		if (reg_rot == 0) or (abs(reg_rot) == 180) {
			...
			return point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), reg_x, reg_y, reg_x + reg_width, reg_y + reg_height);
		} else {
			...
			//Detect mouse in rectangular region
			if (point_in_triangle(device_mouse_x_to_gui(argument[0]), device_mouse_y_to_gui(argument[0]), reg_x0, reg_y0, reg_x1, reg_y1, reg_x2, reg_y2)) 
			or (point_in_triangle(device_mouse_x_to_gui(argument[0]), device_mouse_y_to_gui(argument[0]), reg_x3, reg_y3, reg_x1, reg_y1, reg_x2, reg_y2)) {
				return true;
			} else {
				return false;
			}
		...
		return point_in_circle(device_mouse_x_to_gui(argument[0]), device_mouse_y_to_gui(argument[0]), reg_x, reg_y, reg_width);
	}
