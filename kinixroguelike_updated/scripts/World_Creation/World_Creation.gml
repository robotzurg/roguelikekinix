...
/// @description              Create the randomly generated world.
function create_world(argument0) {
...
#region DOOR CREATION PHASE
				...
				if global.worldgrid[# w,h] mod 2 == 0 { //If there is no wall on the upper area, place a door
					with instance_create_layer((room_size * w) + (room_size/2), (room_size * h) + 32, "Instances", obj_door) {
						image_index = 0; //Vertical
					}
				} else { //Otherwise, place a wall.
					with instance_create_layer((room_size * w) + (room_size/2), (room_size * h) + 32, "Instances", obj_doorfill) {
						image_index = 0; //Up
					}
				}
				...
				if global.worldgrid[# w,h] mod 5 == 0 { //If there is no wall on the lower area, place a door
					with instance_create_layer((room_size * w) + (room_size/2), (room_size * h) + 928, "Instances", obj_door) {
						image_index = 0; //Vertical
					}
				} else { //Otherwise, place a wall.
					with instance_create_layer((room_size * w) + (room_size/2), (room_size * h) + 928, "Instances", obj_doorfill) {
						image_index = 1; //Down
					}
				}
				...
				if global.worldgrid[# w,h] mod 7 == 0 { //If there is no wall on the left area, place a door
					with instance_create_layer((room_size * w) + 32, (room_size * h) + (room_size/2), "Instances", obj_door) {
						image_index = 1; //Horizontal
					}
				} else { //Otherwise, place a wall.
					with instance_create_layer((room_size * w) + 32, (room_size * h) + (room_size/2), "Instances", obj_doorfill) {
						image_index = 2; //Left
					}
				}
				...
				if global.worldgrid[# w,h] mod 3 == 0 { //If there is no wall on the right area, place a door
					with instance_create_layer((room_size * w) + 928, (room_size * h) + (room_size/2), "Instances", obj_door) {
						image_index = 1; //Horizontal
					}
				} else { //Otherwise, place a wall.
					with instance_create_layer((room_size * w) + 928, (room_size * h) + (room_size/2), "Instances", obj_doorfill) {
						image_index = 3; //Right
					}
				}
