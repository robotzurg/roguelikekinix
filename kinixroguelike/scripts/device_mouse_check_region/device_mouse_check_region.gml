...
/*
Checks whether the mouse is currently within a certain region relative to room
...
argument1 = horizontal room coordinate of the region to check (real)
argument2 = vertical room coordinate of the region to check (real)
...
if (reg_shape == reg_shape_rectangle) {
	if (reg_rot == 0) or (abs(reg_rot) == 180) {
		...
		return point_in_rectangle(device_mouse_x(0), device_mouse_y(0), reg_x, reg_y, reg_x + reg_width, reg_y + reg_height);
	} else {
		...
		//Detect mouse in rectangular region
		if (point_in_triangle(device_mouse_x(argument[0]), device_mouse_y(argument[0]), reg_x0, reg_y0, reg_x1, reg_y1, reg_x2, reg_y2)) 
		or (point_in_triangle(device_mouse_x(argument[0]), device_mouse_y(argument[0]), reg_x3, reg_y3, reg_x1, reg_y1, reg_x2, reg_y2)) {
			return true;
		} else {
			return false;
		}
	...
	return point_in_circle(device_mouse_x(argument[0]), device_mouse_y(argument[0]), reg_x, reg_y, reg_width);
}