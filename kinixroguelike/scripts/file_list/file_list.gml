...
/*
Scans a directory and returns the contents as a ds_list, including relative paths
...
If the attribute filter is supported and set to fa_directory, only directories will 
be returned. This is the only supported filter, and all other options will return
a list of files instead.

Note that for best processing speed, files without extensions are not supported by
this script.

Also note that by default, this script can only be used to scan directories within 
working_directory or elsewhere previously granted access via the get_save_filename 
function. On desktop platforms, this limitation can be removed by disabling the 
filesystem sandbox in Game Settings.

argument0 = the full path of the target directory, including drive letter (string)
argument1 = enables filtering directories (fa_directory) or files (anything else) (integer/constant)
argument2 = enables including subdirectories in scan results (boolean) (optional, use no argument for false)
...
var del_count,
	...
	dxindex = 0,
	dyindex = 1,
	plength = string_length(argument[0]) + 1;
...
while (dlist[| dxindex] != "") {
	...
	if (filename_is_dir(dlist[| dxindex])) {
		...
		if (filename_dir(dlist[| dxindex]) == argument[0]) {
...
for (dyindex = ds_list_size(dlist) - 1; dyindex >= 0; dyindex--) {
	...
	if (argument[1] == fa_directory) xor (filename_is_dir(dlist[| dyindex])) {
